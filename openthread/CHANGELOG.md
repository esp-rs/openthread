...
## Unreleased
* (Breaking) Changes to the Radio trait to fill-in functiinality gaps, fix bugs and bring more clarity (#109)
  * API for fetching initial TX power (inb dBm) and CCA energy detect threshold (in dBm)
...
* Run upstream OpenThread timing and functional E2E tests against `openthread`-derived binaries, for testing `openthread`'s async plumbing and the Rust 802.15.4 drivers (#109)
  * New `tests` (crate `openthread-tests`) sub-project containing the test binaries excercised during E2E tests
  * The `xtask` sub-project CLI can now run the E2E tests
  * E2E tests excercised as part of the CLI
